...
public class BlockUserServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int i = dao.blockUser(userId);

            if (i > 0) {
                out.println("<script>alert('User Blocked Successfully'); window.location='ViewUsersServlet';</script>");
            } else {
                out.println("<script>alert('User Not Blocked'); window.location='ViewUsersServlet';</script>");
            }
